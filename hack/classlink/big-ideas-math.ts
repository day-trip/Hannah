...
type BIMQuestion = {
    ...
    axis_x?: {
        ...
        ticks_distance: number
        show_first_arrow: boolean
        show_last_arrow: boolean
        axis_label: string
        draw_labels: boolean
    }
    axis_y?: {
    ...
    data: {
        ...
        request: {
            ...
            features: {
                ...
                liveTutor: boolean
                needHelp: boolean
                checkAnswerCounts: Record<string, number>
                desmos: boolean
                checkLimit: number
                guidedSolution: boolean
                desmosType: string
                viewSolution: boolean
                releaseRev: boolean
            };
            user_id: string
            liveTutorArray: Record<string, boolean>
            name: string
            activity_id: string
            state: string
            ...
            config: {
                ...
                regions: string
                region_overrides: {
                    "top-right.pause_button": boolean
                    "items.progress_element": boolean
                    "top-right": {
                        position?: string
                        type: string
                        show_label_option?: string
                        options?: {
                            name: string
                            icon_class: string
                            label: string
                    ...
                    right: {
                        ...
                        type: string
                        fontsize_option?: boolean
                        zoom_option?: boolean
                        colorscheme_option?: boolean
                    }[]
                }
                configuration: {
                    fontsize: string
                ...
                metadata: {
                    ...
                    items_api_version: string
                    activity_template_id: string
                    current_time: number
                    current_reading_time: boolean
                    max_time: number
                    current_item_reference: string
                    current_sheet_position: number
                    features: any[]
                    user_agent: string
                    last_item_reference: string
...
class BIMAssignment {
    private readonly data: ActivityResponse;
    private readonly security: Security;
    private readonly parent: BigIdeasMath;

    constructor(data: ActivityResponse, security: Security, parent: BigIdeasMath) {
        this.data = data;
        this.security = security;
        this.parent = parent;
    }

    private getQuestionResponse(question: BIMQuestion): [string, any] {
        if (question.type === "graphplotting") {
            const actions = [];
            for (const x of question.validation.valid_response.value) {
                const {id, type, ...rest} = x;
                const args = type === "polygon" ? Object.values(rest.subElementsIds).join(", ") : Object.values(rest.coords).join(",");
                actions.push(`${type}(${args}) << id: '${id}', ${type === "polygon" ? "recursionDepthHigh: 13, recursionDepthLow: 8" : "snaptogrid: true, snapsizex: 1, snapsizey: 1"} >>`);
            }
            return ["object", {
                "actions": actions.join(";") + ";",
                "undo": question.validation.valid_response.value.map(x => `remove(${x.id})`).join(";") + ";",
        ...
        throw new Error(`The question type ${question.type} is unknown!`);
    }

    public async DO_NOT_CALL_THIS() {
        // TODO: this function destroys your entire bim :)
        const items = this.data.data.request.config.metadata.items.map(x => ({
            ...x,
            attempt_status: "fully_attempted"
        }));
        const questions = this.data.data.apiActivity.items.flatMap(x => x.questions);
        const body = {
            "submit": false, //
            ...
            "questionResponses": questions.map(question => {
                const [type, data] = this.getQuestionResponse(question);
                return {
                    id: `0258_${this.security.user_id}_${question.response_id}`,
                    response: {
                        "value": data,
                        "type": type,
                        "apiVersion": "v2.203.2",
        ...
        const response = await this.parent.request("update", "https://questions.learnosity.com/latest/questionresponses?", {"consumer_key":"8EO6ataGL8M5Tuxx","domain":"www.bigideasmath.com","timestamp":this.data.data.apiActivity.questionsApiActivity.timestamp,"user_id":"USRstudent37941401101061","signature":this.data.data.apiActivity.questionsApiActivity.signature}, body, "usrequest", false);
...
class BigIdeasMath {
    ...
    private constructor(session: string, key: string) {
        this.session_id = session;
        this.consumer_key = key;
    }

    public static from(as: AppSession): BigIdeasMath {
        const cookies: Record<string, string> = Object.fromEntries(as.headers.getSetCookie().map(x => ([x.split(";")[0].split("=")[0], x.split(";")[0].split("=")[1]])));
        const session = cookies["BIM.SESSION.ID"];
        console.log("Session:", session);
        return new BigIdeasMath(session, "8EO6ataGL8M5Tuxx"); // LWSD consumer key???
    }

    public async request(method: string, url: string, security: Security, request: any, key: string = "request", timestamp: boolean = true): Promise<any> {
        ...
        const queryString = Array.from(formData.entries()).map(pair => pair[0] + '=' + encodeURIComponent(String(pair[1]))).join('&').replace("%20", "+");

        const r = await fetch(`${url}?&a=${method}&c=${this.consumer_key}&s=${request.session_id}`, {
            headers: {
                "accept": "*/*",
    ...
    public async getActivity(request: ActivityRequest): Promise<BIMAssignment> {
        const {request: r, security} = await this.activityInit(request);
        const response = await this.request("get", "https://items.learnosity.com/v2023.1.LTS/activity", security, r);
        return new BIMAssignment(response, security, this);
    }

    private async activityInit(request: ActivityRequest): Promise<{ request: any, security: Security }> {
        const r = await fetch(`https://www.bigideasmath.com/MRL/rest/learnosity/assignment-init?assignmentId=${request.id}`, {
            headers: {
                ...
                "sec-fetch-mode": "cors",
                "sec-fetch-site": "same-origin",
                "cookie": `BIM.SESSION.ID=${this.session_id};`,
        ...
        return (await r.json()).learnosity;
    }
}

class Skyward {
    private readonly data: SkywardSession;

    private constructor(data: SkywardSession) {
        this.data = data;
    }

    public static async create(data: SkywardSession): Promise<Skyward> {
        return new Skyward(data);
    }
    ...
    private getQueryString(): string {
        ...
        return Array.from(formData.entries()).map(pair => pair[0] + '=' + encodeURIComponent(String(pair[1]))).join('&');
    }

    private getAttribute($: cheerio.Root, field: string): string {
        return $(`label:contains('${field + ":"}')`).parent().next().html();
    }

    public async getStudentInfo(): Promise<SkywardStudentInfo> {
        const rr = await fetch("https://www2.saas.wa-k12.net/scripts/cgiip.exe/WService=wlkwashs71/sfstudentinfo001.w", {
            "headers": {
        ...
        const gender = this.getAttribute($, "Gender").toLowerCase();
        const student_id = Number.parseInt(this.getAttribute($, "Other ID"));
        const birthday = Date.parse(this.getAttribute($, "Age (Birthday)").split("(")[1].split(")")[0]);
        const family_id = Number.parseInt(this.getAttribute($, "Family ID"));

        const box = $("div").filter((_, e) => $(e).css('display') === 'none').first().children().first();
        const table = box.children("div.sf_gridTableWrap").children().first().children().first().children().toArray();
        const address = $(table[1]).children().first().children().first().html().replace("\n", " ");
        const pgt = $(table[2]).children().first().children().first().children().first().children().first().children("tbody").children("tr").toArray();
        const pgn = box.children("div.sfTag").text().split("(")[0].trimEnd();
        const table2 = $($(table[1]).children("td").toArray()[2]).children("div").first().children("div").first().children("table").first().children("tbody").toArray();
        const pgpp = $($(table2[2]).children().first().children().toArray()[1]).children().first().text().split(" EXT. ");
        console.log(pgpp);
        const pgp = pgpp[0] + " (" + pgpp[1] + ")";

        const raw: [string, string, string, string, string, string][] = pgt.map(p => $(p).children("td").toArray().map(c => $(c).html())).map(([first, ...rest]) => {
            const text = $(first).text().split("(");
            return [text[0].trimEnd(), text[1].split(")")[0].toLowerCase(), ...rest];
        }) as [string, string, string, string, string, string][];


        const guardians = raw.map(([name, role, p1, p2, email, work]) => ({
            name,
            role,
            phone: [p1, p2, pgn === name ? pgp : undefined].map(x => x === "&nbsp;" ? undefined : x).filter(x => !!x),
...
class Classlink {
    private static getQueryParam(url: string, param: string): string {
    ...
    public static async session(username: string, password: string): Promise<ClasslinkSession> {
        const csrf = "HDmLCUPL6i2B-vOXFfDdd3FL";
        const clsession = "s%3A-CAPaYlIt5WHyJ5nWv4b4I2hu6N6pX4J.V%2BwtoVVxsREhCk92GovocNIs9ZQyHRGLmw7WZf9jTPE"; // TODO: figure out how to dynamically generate these

        // return ClasslinkSession.create(csrf, clsession, "b00-1705529071-9bb2b405a2b21ad02587e24ca103e417", "ffe9d6c9-af65-4beb-901e-153b6ec29883");

        const p = await fetch("https://launchpad.classlink.com/login", {
            headers: {
                "accept": "*/*",
                "accept-language": "en-US,en;q=0.9",
                "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
                ...
                "cookie": `_csrf=${csrf}; baseurl=%2Flwsd; clsession=${clsession}`,
                "Referer": "https://launchpad.classlink.com/lwsd",
                "Referrer-Policy": "strict-origin-when-cross-origin"
            },
            body: `username=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}&os=Windows&userdn=&code=lwsd&Browser=Chrome&Resolution=1536x864`,
            method: "POST"
        });

        const {login_url} = await p.json();

        console.log(login_url);

        const pp = await fetch(login_url, {
            headers: {
                "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
                "accept-language": "en-US,en;q=0.9",
                "sec-ch-ua": "\"Not_A Brand\";v=\"8\", \"Chromium\";v=\"120\", \"Google Chrome\";v=\"120\"",
        ...
        const code = Classlink.getQueryParam(pp.url, "code");

        console.log("Code", code);

        const r = await fetch(`https://applications.apis.classlink.com/exchangeCode?code=${code}&response_type=code`, {
            headers: {
                "accept": "application/json, text/plain, */*",
                "accept-language": "en-US,en;q=0.9",
        ...
        const {token, gwsToken} = await r.json();

        console.log(token, gwsToken);

        return await ClasslinkSession.create(csrf, clsession, token, gwsToken);
    }
}

class ClasslinkSession {
    private readonly csrf: string;
    private readonly clsession: string;
    private readonly token: string;
    private readonly gToken: string;
    private apps: App[];

    private constructor(csrf: string, clsession: string, token: string, gToken) {
        this.csrf = csrf;
    ...
    private async init(): Promise<void> {
        const rr = await fetch("https://applications.apis.classlink.com/v1/applicationsPageLoad?", {
            headers: {
                "accept": "application/json, text/plain, */*",
                "accept-language": "en-US,en;q=0.9",
        ...
        const {apps}: { apps: App[] } = await rr.json();
        this.apps = apps.flatMap(app => app.apps || []);
    }

    public static async create(csrf: string, clsession: string, token: string, gToken): Promise<ClasslinkSession> {
        const cs = new ClasslinkSession(csrf, clsession, token, gToken);
        await cs.init();
        return cs;
    }

    private async extractHTMLConfig(html: string): Promise<any> {
        const $ = cheerio.load(html);
    ...
    private async browserSSO(id: number): Promise<{username: string, password: string}> {
        const r = await fetch(`https://launchpad.classlink.com/browsersso/${id}`, {
            headers: {
                "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
                "accept-language": "en-US,en;q=0.9",
                "cache-control": "max-age=0",
                ...
                "sec-fetch-user": "?1",
                "upgrade-insecure-requests": "1",
                "cookie": `_csrf=${this.csrf}; clsession=${this.clsession}; baseurl=%2Flwsd`
            },
        ...
        const {gwstokenMd5, appResponse} = await this.extractHTMLConfig(html);

        const userauth = appResponse.userauth[0];
        const ua = {};

        const cjs = this.getCrypto();
    ...
    public async loadApp(appId: number | "bim" | "skyward"): Promise<BigIdeasMath | Skyward> {
        const app = this.apps.find(x => x.id === appId);
        if (appId === "bim" || appId === 652890) {
            const [url] = app.url;

            console.log("Visiting:", url);

            const bimPage = await fetch(url, {
                headers: {
                    "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
                    "accept-language": "en-US,en;q=0.9",
                    "sec-ch-ua": "\"Not_A Brand\";v=\"8\", \"Chromium\";v=\"120\", \"Google Chrome\";v=\"120\"",
            ...
            const loc = bimPage.headers.get("Location");

            console.log(loc);

            const redr = await fetch(loc, {
                headers: {
                    "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
                    "accept-language": "en-US,en;q=0.9",
                    "sec-ch-ua": "\"Not_A Brand\";v=\"8\", \"Chromium\";v=\"120\", \"Google Chrome\";v=\"120\"",
                    ...
                    "sec-fetch-user": "?1",
                    "upgrade-insecure-requests": "1",
                    "cookie": `_csrf=${this.csrf}; baseurl=%2Flwsd; clsession=${this.clsession}`,
                },
                referrerPolicy: "strict-origin-when-cross-origin",
                method: "GET",
                redirect: "manual",
            ...
            const oauth = redr.headers.get("Location");

            console.log("Final:", oauth);

            const f = await fetch(oauth, {
                headers: {
                    "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
                    "accept-language": "en-US,en;q=0.9",
                    "sec-ch-ua": "\"Not_A Brand\";v=\"8\", \"Chromium\";v=\"120\", \"Google Chrome\";v=\"120\"",
            ...
            return BigIdeasMath.from(f);
        } else if (appId === "skyward" || appId === 1820046) {
            const {username, password} = await this.browserSSO(1820046);
            ...
            const queryString = Array.from(formData.entries()).map(pair => pair[0] + '=' + encodeURIComponent(String(pair[1]))).join('&');

            const r = await fetch("https://www2.saas.wa-k12.net/scripts/cgiip.exe/WService=wlkwashs71/skyporthttp.w", {
                "headers": {
                    "accept": "*/*",
                    "accept-language": "en-US,en;q=0.9",
                    "content-type": "application/x-www-form-urlencoded",
            ...
            const [dwd, web_data_recid, wfaacl_recid, wfaacl, nameid, usr, user_type, _, __, ___, ____, _____, ______, enc, encses, _______, ________] = result.split("<li>")[1].split("</li>")[0].split("^");
            const data = {dwd, web_data_recid, wfaacl_recid, wfaacl, nameid, username: usr, user_type, enc, encses};

            return await Skyward.create(data);
        }
        throw Error(`${appId}: Unimplemented!`);
    }
}

void (async () => {
    // Create your Classlink session
    const session = await Classlink.session("s-jgiri", "Dogs)dogs5");
    const skyward = (await session.loadApp("skyward")) as Skyward;
    const x = await skyward.getStudentInfo();

    console.log(JSON.stringify(x, null, 4));
    // fs.writeFileSync("skibidi.html", x);
    // Login to BIM
    // const bim = BigIdeasMath.from(await session.loadApp("bim"));
    // BOOM! ... Use your BIM session to hack the US government and eat donuts; too ez XD lol
    // const x = await bim.getActivity({id: "c8d17e92-13d4-4906-b7bd-f2baebc6122e"}); // e4cdfcb2-46ac-483c-905b-aaf4dab841b5
    // But to achieve our dreams we first need good grades...
    // await x.answerQuestions();
    // console.log(x);
    // fs.writeFileSync("wato.json", JSON.stringify(x.));
})();
